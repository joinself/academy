...
```bash
./gradlew :02_credentials:02_share_credentials:03_document_credential:installDebug
```
...
5. `Verify Identity Document` before sharing credentials
6. Click on `Start Sharing Document Credential`
...
Before sharing, the credentials must be present in the SDK. Users must first verify their identity document.
```
The SDK initiates an identity verification flow, guiding users to capture the document's images and NFC chip. 
The server verifies the credentials and returns them to the SDK, which stores them for sharing.
```
